...
class ClinicalPreprocessor(BasePreprocessor):
    ...
    def _validate_input(self, image: Any) -> None:
        """Validate input image for security and stability."""
        if image is None:
        ...
        if len(image.shape) not in [2, 3]:
            raise ValueError(f"Input image must have 2 or 3 dimensions, got {len(image.shape)}")

        # Security check: Prevent processing of excessively large images (DoS prevention)
        if any(d > 4096 for d in image.shape[:2]):
             raise ValueError(f"Image dimensions {image.shape} exceed maximum allowed size (4096)")

        # Stability check: NaN/Inf values
