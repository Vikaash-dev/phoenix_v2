...
class TestTrainingImprovements(unittest.TestCase):
    ...
    def test_seed_fixing(self):
        ...
        from src.legacy.training_improvements import set_seed
    ...
    def test_mixed_precision(self):
        ...
        from src.legacy.training_improvements import MixedPrecisionConfig
    ...
    def test_cosine_annealing_scheduler(self):
        ...
        from src.legacy.training_improvements import LearningRateSchedulers
    ...
    def test_kfold_cross_validation(self):
        ...
        from src.legacy.training_improvements import KFoldCrossValidation
    ...
    def test_patient_level_splitting(self):
        ...
        for train_idx, val_idx in splits:
            ...
            self.assertEqual(len(overlap), 0,
                           "Patient-level splitting failed: patients in both sets")
...
class TestONNXDeployment(unittest.TestCase):
    ...
    def test_onnx_export(self):
        ...
        try:
            import tf2onnx
            from src.legacy.onnx_deployment import ONNXExporter
    ...
    @unittest.skip("Skipping TFLite export in test env")
    def test_tflite_export(self):
        """Test TensorFlow Lite export"""
        from src.legacy.onnx_deployment import export_to_tflite
...
class TestCoordinateAttention(unittest.TestCase):
    ...
    def test_coordinate_attention_output_shape(self):
        ...
        from src.models.legacy.coordinate_attention import CoordinateAttentionBlock
    ...
    def test_coordinate_attention_position_encoding(self):
        ...
        # Ensure float32 for this test to avoid underflow
        ...
        try:
            ...
            x = tf.zeros([1, 32, 32, 64], dtype=tf.float32)
            # Use a larger value and more points to ensure variance persists
            indices = [[0, 10, 10, 0], [0, 20, 20, 0]]
            updates = [10.0, 10.0]
            ...
            print(f"CA Variance: {variance.numpy()}")
            self.assertGreater(variance, 1e-6,
                              f"CA output variance {variance} is too low - not preserving position")
        finally:
            # Restore policy (best effort, though usually env specific)
            pass
...
class TestDynamicSnakeConv(unittest.TestCase):
    ...
    def test_snake_conv_output_shape(self):
        ...
        from src.models.legacy.dynamic_snake_conv import DynamicSnakeConv2D
...
class TestDataDeduplication(unittest.TestCase):
    ...
    def test_phash_duplicate_detection(self):
        ...
        # Use temp files
        with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as f1,              tempfile.NamedTemporaryFile(suffix='.png', delete=False) as f2,              tempfile.NamedTemporaryFile(suffix='.png', delete=False) as f3:
            
            f1_name, f2_name, f3_name = f1.name, f2.name, f3.name
        
        try:
            Image.fromarray(img1).save(f1_name)
            Image.fromarray(img2).save(f2_name)
            Image.fromarray(img3).save(f3_name)
            
            # Compute hashes
            hash1 = dedup.compute_phash(f1_name)
            hash2 = dedup.compute_phash(f2_name)
            hash3 = dedup.compute_phash(f3_name)
            ...
            for fname in [f1_name, f2_name, f3_name]:
...
class TestClinicalPreprocessing(unittest.TestCase):
    ...
    def test_skull_stripping(self):
        ...
        from src.legacy.clinical_preprocessing import ClinicalPreprocessing
        ...
        stripped, _ = preprocessor.skull_strip(img)
        
        # Check that skull region is reduced
        self.assertLess(stripped[2, 64], img[2, 64])
