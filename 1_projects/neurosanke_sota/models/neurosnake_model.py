...
class MobileViTBlock(layers.Layer):
    ...
    def call(self, inputs, training=None):
        """Forward pass."""
        batch, height, width, channels = tf.shape(inputs)[0], tf.shape(inputs)[1], tf.shape(inputs)[2], inputs.shape[3]
        ...
        transformer_output = tf.reshape(transformer_output, [batch, height, width, self.filters])
...
def create_neurosnake_with_sevector_attention(
    input_shape=(config.IMG_HEIGHT, config.IMG_WIDTH, config.IMG_CHANNELS),
    num_classes=config.NUM_CLASSES,
    dropout_rate=0.3,
    use_mobilevit=True
    ...
    """
    Create NeuroSnake model with SEVector Attention (Channel Attention).
    
    Alternative to Coordinate Attention. SEVector focuses on "what" (channel recalibration)
    rather than "where" (positional attention).
    
    Args:
        input_shape: Input image shape
        num_classes: Number of output classes
        dropout_rate: Dropout rate
        use_mobilevit: Include MobileViT block
        
    Returns:
        Keras model with SEVector Attention
    """
    if not SEVECTOR_AVAILABLE:
        raise ImportError("SEVector Attention module not available.")
    ...
    # Stage 2: Snake Conv + SEVector (Stride 4)
    x = SnakeConvBlock(64, kernel_size=3, strides=1, dropout_rate=dropout_rate, name='snake_block1')(x)
    x = SEVectorBlock(filters=64, reduction_ratio=8, name='se1')(x)
    x = layers.MaxPooling2D(2, name='pool1')(x)
    
    # Stage 3: Snake Conv + SEVector (Stride 8)
    x = SnakeConvBlock(128, kernel_size=3, strides=1, dropout_rate=dropout_rate, name='snake_block2')(x)
    x = SEVectorBlock(filters=128, reduction_ratio=8, name='se2')(x)
    x = layers.MaxPooling2D(2, name='pool2')(x)
    
    # Stage 4: Snake Conv + SEVector (Stride 16)
    x = SnakeConvBlock(256, kernel_size=3, strides=1, dropout_rate=dropout_rate, name='snake_block3')(x)
    x = SEVectorBlock(filters=256, reduction_ratio=8, name='se3')(x)
    x = layers.MaxPooling2D(2, name='pool3')(x)
    
    # Stage 5: Deepest layer
    x = SnakeConvBlock(512, kernel_size=3, strides=1, dropout_rate=dropout_rate, name='snake_block4')(x)
    ...
    x = SEVectorBlock(filters=512, reduction_ratio=8, name='se4')(x)
    x = layers.MaxPooling2D(2, name='pool4')(x)
    ...
    model = keras.Model(inputs=inputs, outputs=outputs, name='NeuroSnake_SEVector')
