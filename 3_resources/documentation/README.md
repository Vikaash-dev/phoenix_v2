...
## 🔬 Master Research Paper (Consolidated)

**[CONSOLIDATED_RESEARCH.md](CONSOLIDATED_RESEARCH.md)**: This master document synthesizes the entire project lineage, documenting the evolution from the geometric baseline to the **Hyper-Liquid Snake** architecture. It includes:

*   Architectural Phylogeny: 5 stages of evolution (ViT, Spectral, KAN, TTT, Liquid).
*   Grand Benchmark: Quantitative comparison of accuracy, latency, and robustness.
*   Security Hardening: Mitigation strategies for "Med-Hammer" (Rowhammer) attacks.
*   Industrial Alignment: Benchmarks against **MONAI** and **nnU-Net**.
*   Strategic Roadmap: Official adoption of **Self-Supervised Disentangled Representations**.
...
# Train with Phoenix Protocol enhancements
python src/train_phoenix.py

# Train with one-click interface
python one_click_train_test.py --mode train --model-type neurosnake_ca --deduplicate

# 4. Deploy to edge with EfficientQuant
python src/efficient_quant.py --model-path models/baseline_cnn_model.h5 --output quantized_model.tflite
python src/int8_quantization.py --model-path models/baseline_cnn_model.h5 --output quantized_model.tflite
```
...
1. Supported image formats: `.jpg`, `.jpeg`, `.png`, `.bmp`
2. Images will be automatically resized to 224×224 pixels
...
| Metric | Baseline | NeuroSnake |
|--------|----------|------------|
| Total Parameters | ~8.2 million | ~15 million |
| Model Size (INT8) | ~32 MB | ~40 MB |
| Inference Time (GPU) | ~50ms | ~80ms |
