...
if __name__ == "__main__":
    ...
    try:
        # Import data preprocessing specifically for this script
        from src.data_preprocessing import load_dataset_from_directory
        
        # Load calibration data (from validation or train set)
        calib_dir = os.path.join(args.data_dir, 'validation')
        if not os.path.exists(calib_dir):
            calib_dir = os.path.join(args.data_dir, 'train')
            
        print(f"Using {calib_dir} for calibration...")
        
        # Create calibration dataset
        calib_ds = tf.keras.utils.image_dataset_from_directory(
            calib_dir,
            image_size=(224, 224),
            batch_size=1,  # Batch size 1 for calibration generator
            shuffle=True,
            label_mode='categorical'
        )
        # Normalize to [0,1]
        calib_ds = calib_ds.map(lambda x, y: (tf.cast(x, tf.float32) / 255.0, y))
        ...
        if os.path.exists(test_dir):
            print(f"Using {test_dir} for validation...")
        ...
        print("Error: Could not import data loading utilities.")
