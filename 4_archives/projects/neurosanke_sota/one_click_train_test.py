...
# Add project and shared area to path for imports
project_root = Path(__file__).resolve().parent
sys.path.insert(0, str(project_root))
shared_root = project_root.parent.parent / '2_areas' / 'main_app'
...
def train_model(args):
    ...
    if args.model_type == 'neurosnake':
        ...
        print("  Creating NeuroSnake + Coordinate Attention model...")
    ...
    elif args.model_type == 'neurokan':
        print("  Creating NeuroKAN model (SOTA 2025)...")
        print("  ✓ Kolmogorov-Arnold Networks + Snake Convolutions + Boundary Loss")
    ...
    if args.model_type == 'neurokan':
        # Use Compound Loss for NeuroKAN
        ...
        loss = compound_loss
        loss_name = "Compound (Focal + LogCoshDice + Boundary)"
    else:
        loss = create_focal_loss(alpha=0.25, gamma=2.0)
        loss_name = "Focal Loss (α=0.25, γ=2.0)"
...
Examples:
  ...
  python one_click_train_test.py --mode train --model-type baseline --epochs 50
    ...
    # Training arguments
    parser.add_argument('--model-type', type=str, default='neurokan',
                      choices=['neurosnake', 'neurosnake_ca', 'neurokan', 'baseline'],
                      help='Model architecture: neurosnake, neurosnake_ca, neurokan (SOTA), baseline')
