...
class DynamicSnakeConv2D(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.offset_conv = layers.Conv2D(
            ...
            kernel_regularizer=keras.regularizers.l2(1e-4), # Regularize offsets to prevent extreme deformation
            name='offset_conv'
        )
    ...
    def _deformable_conv2d(self, inputs, offsets, modulation):
        ...
        base_grid = tf.cast(base_grid, offsets.dtype)
    ...
    def _get_base_grid(self, height, width, kernel_h, kernel_w):
        ...
        # kernel_grid is (kernel_h * kernel_w, 2)
        # We want (1, height, width, kernel_h * kernel_w, 2)
    ...
    def _bilinear_sample(self, inputs, grid):
        ...
        # Clip to valid range
        y = tf.clip_by_value(y, 0.0, tf.cast(height - 1, y.dtype))
        x = tf.clip_by_value(x, 0.0, tf.cast(width - 1, x.dtype))
        ...
        wy1 = y - tf.cast(y0, y.dtype)
        wy0 = 1.0 - wy1
        wx1 = x - tf.cast(x0, x.dtype)
        wx0 = 1.0 - wx1
        ...
        # Gather values
        v00 = tf.gather(inputs_flat, idx_00)
        v01 = tf.gather(inputs_flat, idx_01)
        v10 = tf.gather(inputs_flat, idx_10)
        v11 = tf.gather(inputs_flat, idx_11)
