...
class PhysicsInformedAugmentation:
    ...
    def _get_random_state(self, random_state: Optional[Union[np.random.RandomState, object]]) -> object:
        """
        Helper to get a random state object.
        If random_state is None, returns the global np.random module.
        This avoids expensive RandomState instantiation and ensures global seeding is respected.
        """
    ...
    def elastic_deformation(
        ...
        if alpha is None:
            alpha = rng.uniform(*self.elastic_alpha_range)
    ...
    def rician_noise(
        ...
        if sigma is None:
            sigma = rng.uniform(*self.rician_noise_sigma_range)
    ...
    def ghosting_artifact(
        ...
        # We need a copy because we accumulate changes
        result = image.copy()
        
        for _ in range(num_ghosts):
            # Random shift amount
            shift_y = rng.randint(-image.shape[0] // 20, image.shape[0] // 20)
            shift_x = rng.randint(-image.shape[1] // 20, image.shape[1] // 20)
    ...
    def augment(
        ...
        # ⚡ Optimization: Avoid initial copy if possible.
        # However, augmentations like elastic deformation return a new array anyway.
        # Only ghosting and in-place modifications would benefit significantly.
        # Given safety is paramount, we keep copy but rely on efficient RNG.
        result = image.copy()
        
        # Apply elastic deformation (most important for irregular tumor boundaries)
        if apply_elastic and rng.rand() < self.apply_probability:
            result = self.elastic_deformation(result, random_state=random_state)
        
        # Apply Rician noise (MRI-specific)
        if apply_rician and rng.rand() < self.apply_probability:
            result = self.rician_noise(result, random_state=random_state)
        
        # Apply intensity inhomogeneity (bias field)
        if apply_inhomogeneity and rng.rand() < self.apply_probability * 0.5:
            result = self.intensity_inhomogeneity(result, random_state=random_state)
        
        # Apply ghosting (less common, so lower probability)
        if apply_ghosting and rng.rand() < self.apply_probability * 0.3:
